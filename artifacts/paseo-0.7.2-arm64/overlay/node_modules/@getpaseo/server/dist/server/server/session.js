...
import { sendPromptToAgent, waitForAgentRunStartWithTimeout, unarchiveAgentState, } from "./agent/agent-prompt.js";
...
import { WorkspaceLabelError, WorkspaceLabelStorageUncertainError, } from "./workspace-labels/index.js";
...
import { ProjectIconReader, removeProjectCustomIcon, setProjectCustomIcon, } from "../utils/project-custom-icon.js";
...
function resolveDirectorySync(service) {
 ...
 */
function resolveWorkspaceLabelService(service) {
    return service ?? null;
}
function workspaceLabelErrorCode(error) {
    if (error instanceof WorkspaceLabelError ||
...
export class Session {
    constructor(options) {
        ...
        this.projectUpdateQueue = Promise.resolve();
        ...
        const { clientId, permissions, appVersion, clientCapabilities, onMessage, onMessageToSource, onBinaryMessage, onBinaryMessageToSource, getTransportBufferedAmount, onLifecycleIntent, onWorkspaceRecovered, logger, downloadTokenStore, pushNotifications, paseoHome, worktreesRoot, agentManager, agentStorage, projectRegistry, workspaceRegistry, directorySync, workspaceLabelService, filesystem, scheduleService, checkoutDiffManager, github, renameCurrentBranch, workspaceGitService, workspaceAutoName, daemonConfigStore, pluginRuntime, orchestrationSkills, stt, sttLanguage, tts, terminalManager, providerSnapshotManager, providerUsageService, serviceProxy, scriptRuntimeStore, workspaceSetupSnapshots, workspaceSetupRuntime, onBranchChanged, getDaemonTcpPort, getDaemonTcpHost, serviceProxyPublicBaseUrl, resolveScriptHealth, voice, voiceBridge, dictation, serverId, daemonVersion, daemonRuntimeConfig, getWebSocketRuntimeMetrics, } = options;
        ...
        this.providerCatalogSession = new ProviderCatalogSession({
            host: {
                ...
                getAgentProviderUsage: async (agentId) => {
                    await ensureUnarchivedAgentLoaded(agentId, {
                        agentManager,
                        agentStorage,
                        logger: this.sessionLogger,
                    });
                    return this.agentManager.getProviderUsage(agentId);
                },
            },
            providerSnapshotManager,
        ...
        this.hubExecutionController = options.hubExecutionAgents
            ? new HubExecutionController({
                ...
                validateAgentConfiguration: (input) => providerSnapshotManager.validateAgentConfiguration(input),
        ...
        this.agentUpdates = createAgentUpdatesService({
            ...
            emitWorkspaceUpdateForWorkspaceId: (workspaceId) => this.emitWorkspaceUpdateForWorkspaceId(workspaceId),
            sequenceAgentUpdate: (payload, agent, project, agentId, includeSequence) => this.directorySync.sequenceAgentUpdate(payload, agent && project ? { agent, project } : null, agentId, includeSequence),
            logger: this.sessionLogger,
        });
    ...
    async publishProjectUpdate(update) {
        const projectedPayload = update.kind === "upsert"
    ...
    subscribesToAgent(agent) {
        return this.agentUpdates.includesLiveAgent(agent);
    }
    subscribesToTerminalDirectory(input) {
    ...
    dispatchOrchestrationSkillsMessage(msg) {
        if (!this.orchestrationSkills || !msg.type.startsWith("agent.skills."))
            return undefined;
        const emitStatus = (type, requestId, operation) => operation.then((status) => {
            this.emit({ type, payload: { requestId, ...status } });
            return undefined;
        });
        switch (msg.type) {
            case "agent.skills.get_status.request":
                return emitStatus("agent.skills.get_status.response", msg.requestId, this.orchestrationSkills.getStatus());
            case "agent.skills.reconcile.request":
                return emitStatus("agent.skills.reconcile.response", msg.requestId, this.orchestrationSkills.reconcile());
            case "agent.skills.uninstall.request":
                return emitStatus("agent.skills.uninstall.response", msg.requestId, this.orchestrationSkills.uninstall());
    ...
    dispatchPluginMessage(msg) {
        ...
        if (msg.type === "plugin.logs.get.request") {
            if (!this.pluginRuntime)
                throw new Error("Plugin service is unavailable");
            this.emit({
    ...
    dispatchPluginDirectoryMessage(msg) {
    ...
    subscribeToPluginChanges(pluginRuntime) {
        if (!pluginRuntime)
            return null;
    ...
    dispatchAgentRewindMessage(msg, source) {
        switch (msg.type) {
    ...
    dispatchWorkspaceLabelMessage(msg) {
        switch (msg.type) {
    ...
    async unarchiveAgentByHandle(handle) {
        const records = await this.agentStorage.listByProviderSession(handle.provider, handle.sessionId);
        const matched = records.reduce((latest, candidate) => {
            if (!latest) {
                return candidate;
            }
    ...
    async handleAgentRewindRequest(msg, source) {
        try {
    ...
    deliverTimelineReplacement(agentId, initiatingSource) {
        const agent = this.agentManager.getAgent(agentId);
        if (!agent)
            return;
    ...
    emitReconstructedTimelineRows(agentId, provider, rows, epoch, source) {
    ...
    async buildProjectDescriptor(project) {
    ...
    async emitWorkspaceUpdateBatch(workspaceIds, subscription, options) {
        ...
        for (const workspaceId of workspaceIds) {
            ...
            if (!nextWorkspace) {
                ...
                const removePayload = await this.buildWorkspaceRemoveUpdatePayload(workspaceId, options?.removedProjectId);
                this.bufferOrEmitWorkspaceUpdate(subscription, this.directorySync.sequenceWorkspaceUpdate(removePayload, workspace ?? null, workspaceId, subscription.syncEnabled === true));
                continue;
            }
            const nextPayload = this.directorySync.sequenceWorkspaceUpdate({ kind: "upsert", workspace: nextWorkspace }, workspace ?? null, workspaceId, subscription.syncEnabled === true);
            if (lastEmitted &&
    ...
    async handleProjectListRequest(request) {
        try {
            const projects = await Promise.all((await this.projectRegistry.list())
                .filter((project) => !project.archivedAt)
    ...
    emitWorkspaceLabelError(request, error) {
        this.emit({
            type: "rpc_error",
            payload: {
                requestId: request.requestId,
    ...
    async handleWorkspaceLabelList(request) {
        const owner = {};
        try {
            ...
            if (ownsSubscription) {
                for (const change of pending) {
                    if (change.seq > subscription.snapshot.sync.headSeq)
                        emitChange(change);
                }
            }
        }
    ...
    async handleWorkspaceLabelAssignment(request) {
        try {
    ...
    async handleWorkspaceLabelUpdate(request) {
        try {
    ...
    async handleWorkspaceLabelDelete(request) {
        try {
    ...
    async handleWorkspaceLabelDeleteInspection(request) {
        try {
            const affectedWorkspaceCount = await this.requireWorkspaceLabels().countAffectedWorkspaces(request.name);
            this.emit({
    ...
    async readAgentDirectorySync(request) {
        if (request.scope !== "active" || request.filter) {
            throw new SessionRequestError("invalid_request", "Sequenced agent directory reads require scope=active and no filter.");
        }
    ...
    async readWorkspaceDirectorySync(request) {
        if (request.filter) {
            throw new SessionRequestError("invalid_request", "Sequenced workspace directory reads do not support filters.");
        }
        return this.directorySync.synchronizeWorkspaces(await this.workspaceDirectory.listDescriptors(), request.sync ?? {});
    }
