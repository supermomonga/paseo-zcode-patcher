...
export function requireMode(mode) {
    if (!ZCODE_MODES.some((candidate) => candidate.id === mode)) {
        throw new AdapterError("NATIVE_PROTOCOL_ERROR", `ZCode returned an unknown mode: ${mode}`);
    }
    return mode;
}
...
export function catalogModels(settings) {
    const ids = new Set();
    ...
    const models = settings.model.available.map((model) => {
        ...
        if (ids.has(id)) {
            throw new AdapterError("NATIVE_PROTOCOL_ERROR", `Duplicate ZCode model ID: ${id}`);
        }
        ids.add(id);
        return {
    ...
    if (!ids.has(current)) {
        throw new AdapterError("NATIVE_PROTOCOL_ERROR", "The current ZCode model is absent from the available model list");
    }
    requireMode(settings.mode.current);
    return models;
}
function catalogThinkingOptions(settings) {
    ...
    if (defaultOptionId === undefined ||
        ...
        throw new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode returned an invalid default thinking option");
    }
    return {
        defaultOptionId,
...
export function mapMcpServers(servers) {
    return Object.entries(servers ?? {}).map(([name, server]) => {
        if (name.trim() === "") {
            throw new AdapterError("INVALID_CONFIGURATION", "MCP server name must not be empty");
        }
        if (server.alwaysLoad !== undefined) {
            throw new AdapterError("INVALID_CONFIGURATION", `MCP server '${name}' uses unsupported alwaysLoad`);
        }
        if (server.type === "stdio") {
            if (!isAbsolute(server.command)) {
                throw new AdapterError("INVALID_CONFIGURATION", `MCP server '${name}' command must be absolute`);
            }
            return {
                name,
        ...
        const headers = Object.entries(server.headers ?? {}).map(([key, value]) => ({
            name: key,
            value,
        }));
        ...
        throw new AdapterError("INVALID_CONFIGURATION", `Unsupported MCP server: ${String(unreachable)}`);
    });
}
export async function mapPrompt(prompt, workspace) {
    if (typeof prompt === "string")
        return { content: prompt, attachments: [] };
    ...
    for (const block of prompt) {
        ...
        if (block.type === "uploaded_file") {
            if (!isAbsolute(block.path)) {
                throw new AdapterError("UNSUPPORTED_CONTENT", "Attachment path must be absolute");
            }
            const resolved = await realpath(block.path);
            const child = relative(canonicalWorkspace, resolved);
            if (child.startsWith("..") || isAbsolute(child)) {
                throw new AdapterError("UNSUPPORTED_CONTENT", "Attachment path must remain inside the workspace");
            }
            const info = await stat(resolved);
            if (!info.isFile()) {
                throw new AdapterError("UNSUPPORTED_CONTENT", "Attachment must be a regular file");
            }
            attachments.push({
        ...
        throw new AdapterError("UNSUPPORTED_CONTENT", `Unsupported Paseo attachment type: ${block.type}`);
    }
    return { content: content.join("\n\n"), attachments };
}
export function historyTimeline(snapshot) {
    const result = [];
        ...
        for (const part of message.parts) {
            ...
            if (part.type === "text" || part.type === "reasoning") {
                if (typeof part.text !== "string") {
                    throw new AdapterError("NATIVE_PROTOCOL_ERROR", "Persisted text part is malformed");
                }
                result.push(part.type === "reasoning"
            ...
            if (part.type === "file") {
                if (typeof part.url !== "string") {
                    throw new AdapterError("NATIVE_PROTOCOL_ERROR", "Persisted file part is malformed");
                }
                const label = typeof part.filename === "string" ? part.filename : "file";
                const text = `[${label}](${part.url})`;
                result.push(message.info.role === "user"
                ...
                const status = state.status === "completed"
                    ...
                    : state.status === "error"
                        ...
                        : state.status === "running" || state.status === "pending"
                            ...
                            : undefined;
                if (status === undefined) {
                    throw new AdapterError("NATIVE_PROTOCOL_ERROR", "Persisted tool status is unknown");
                }
                const output = state.output ?? state.error ?? null;
                const detail = {
                    type: "unknown",
                    input: state.input,
                    output,
            ...
            throw new AdapterError("NATIVE_PROTOCOL_ERROR", `Unsupported persisted message part: ${part.type}`);
        }
    }
    if (snapshot.todos !== undefined) {
        result.push({
            type: "todo",
            items: snapshot.todos.map((todo) => Object.assign({
                text: todo.content,
...
export function mapQuestionRequest(request, id) {
    if (request.questions === undefined || request.questions.length === 0) {
        throw new AdapterError("INTERACTION_UNSUPPORTED", "ZCode question has no fields");
    }
    const headers = new Set();
    const fields = request.questions.map((question) => {
        if (headers.has(question.header)) {
            throw new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode question headers must be unique");
        }
        ...
        for (const option of question.options) {
            if (option.label.trim() === "" ||
                ...
                throw new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode question options are not reversibly representable");
            }
            valuesByLabel.set(option.label, option.value);
            values.add(option.value);
        }
        if (valuesByLabel.size === 0) {
            throw new AdapterError("INTERACTION_UNSUPPORTED", "ZCode free-form question is unsupported by this native contract");
        }
        return {
...
export function questionContent(fields, answers) {
    ...
    fields.forEach((field, index) => {
        ...
        if (typeof answer !== "string" || answer === "") {
            throw new AdapterError("NATIVE_PROTOCOL_ERROR", `Missing answer for ZCode question '${field.header}'`);
        }
        const labels = field.multiSelect ? answer.split(", ") : [answer];
        if (labels.length === 0 ||
            new Set(labels).size !== labels.length ||
            labels.some((label) => label === "")) {
            throw new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode answer is ambiguous");
        }
        const values = labels.map((label) => {
            const value = field.valuesByLabel.get(label);
            if (value === undefined) {
                throw new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode answer contains an unknown option");
            }
            return value;
        });
        ...
        if (index === 0)
            content.answer = field.multiSelect ? values : values[0];
    });
    content.answers = nativeAnswers;
    return content;
}
export function permissionActions(request, requireBoth = false) {
    let allow = false;
    let deny = false;
    const actions = request.options.map((option) => {
        if (option.response.decision !== "allow" &&
            option.response.decision !== "deny") {
            throw new AdapterError("INTERACTION_UNSUPPORTED", `ZCode permission decision is unsupported: ${option.response.decision}`);
        }
        allow || (allow = option.response.decision === "allow");
        deny || (deny = option.response.decision === "deny");
        return {
            id: option.optionId,
            label: option.name,
    ...
    if (!deny || (requireBoth && !allow)) {
        throw new AdapterError("NATIVE_PROTOCOL_ERROR", requireBoth
            ? "ZCode plan approval must provide allow and deny options"
            : "ZCode permission must provide a deny option");
    }
    return actions;
}
export function planMarkdown(input) {
    const plan = record(input).plan;
    if (typeof plan !== "string" || plan.trim() === "") {
        throw new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode plan approval has no Markdown");
    }
    return plan.trim();
}
