...
import { resolvedHostMismatch, resolveHostContractPaths } from "./host-contract.js";
    ...
    return await new Promise((resolve, reject) => {
        ...
        child.stdout.on("data", (chunk) => {
            if (stdout.length + chunk.length > 1024 * 1024) {
                child.kill("SIGTERM");
                reject(new AdapterError("NATIVE_PROTOCOL_ERROR", "ZCode inspection output is too large"));
                return;
            }
            stdout += chunk;
        });
        ...
        child.once("exit", (code, signal) => {
            if (signal !== null) {
                reject(new AdapterError("NATIVE_EXITED", `ZCode runtime terminated by ${signal}`));
                return;
            }
...
export async function discoverRuntime(options = {}) {
    ...
    if (platform !== "darwin" || architecture !== "arm64") {
        throw new AdapterError("UNSUPPORTED_PLATFORM", "ZCode supports only darwin-arm64");
    }
    ...
    try {
        ...
        throw new AdapterError("RUNTIME_DISCOVERY_FAILED", "ZCode install root does not exist", {}, { cause: error });
    }
    const paths = resolveRuntimePaths(installRoot);
    await validatePaths(paths);
    const metadataValue = JSON.parse(await readFile(paths.metadata, "utf8"));
    const bundle = BundleMetadataSchema.parse(metadataValue);
    const environment = {
    ...
    const cliVersion = cliResult.exitCode === 0
    ...
    const host = assessment.hostArtifact === undefined || assessment.hostProtocol === undefined
        ? undefined
        : await resolveHost(paths, assessment.hostArtifact, assessment.hostProtocol, environment, options.signal);
    const mismatch = host === undefined
        ? undefined
...
export async function runRuntimeSmoke(runtime, environment = process.env, signal) {
    try {
        ...
        const [version, doctor] = await Promise.all([
            ...
            run(runtime.paths.executable, [runtime.paths.cliEntry, "doctor", "--json"], {
                cwd: runtime.paths.installRoot,
                environment: commandEnvironment,
                ...(signal === undefined ? {} : { signal }),
            }),
...
export function assertRuntimeSupported(runtime) {
...
function resolveRuntimePaths(installRoot) {
    return {
        installRoot,
        executable: join(installRoot, "Contents/Frameworks/ZCode Helper.app/Contents/MacOS/ZCode Helper"),
...
async function validatePaths(paths) {
    try {
        ...
        if (error instanceof AdapterError)
            throw error;
        throw new AdapterError("RUNTIME_DISCOVERY_FAILED", "Required ZCode runtime files are unavailable", {}, { cause: error });
    }
}
function ensureInside(root, candidate) {
    const child = relative(root, candidate);
    if (child.startsWith("..") || isAbsolute(child)) {
        throw new AdapterError("RUNTIME_DISCOVERY_FAILED", "Resolved runtime path is outside the install root");
    }
}
...
async function readPlist(file, key, signal) {
    const result = await run("/usr/libexec/PlistBuddy", ["-c", `Print :${key}`, file], {
        cwd: dirname(file),
        environment: process.env,
        ...(signal === undefined ? {} : { signal }),
    });
    if (result.exitCode !== 0 || result.stdout.trim() === "") {
        throw new AdapterError("RUNTIME_DISCOVERY_FAILED", `ZCode Info.plist is missing ${key}`);
    }
    return result.stdout.trim();
}
async function resolveHost(paths, artifact, protocol, environment, signal) {
    const { hostIndex, hostRpcModule } = resolveHostContractPaths(paths.installRoot, paths.hostArchive, artifact);
...
(async () => {
  ...
  process.stdout.write(JSON.stringify({
    ...
    if (result.exitCode !== 0) {
        throw new AdapterError("RUNTIME_DISCOVERY_FAILED", "Failed to inspect the manifest-selected ZCode host artifact");
    }
