...
ZCodeセッションでも、マイク左の円アイコンからコンテキスト使用量と使用中の接続先の個人契約クオータを確認できます。公式hostの現在値を使用し、圧縮・モデル変更・再開に追従します。接続先名、プラン、使用率、残量、リセット時刻を既存カードへ表示し、未対応・取得失敗は明示します。Team Planは対象外です。生成アプリとZCode公式画面で数値を照合済みです。追加のリセット残数・期限表示と、5時間→週間→月間ツールの並び順も実装済みです（アプリ反映状況は実装状況を参照）。
