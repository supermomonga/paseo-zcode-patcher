...
Coding Planのリセット権は公式`getCodingPlanResetStatus`を同じ接続先で読み、期限切れを除いた枠別の回数と最短期限（端末のローカル時刻・UTCオフセット付き）を詳細行へ渡す。ゼロ件と取得失敗を区別し、読み取り失敗でも通常クオータは保持する。リセットを消費するメソッドは公開しない。[ADR 0014](adr/0014-公式hostからリセット権の残数と期限を読み取る.md)を参照する。

使用量・クオータ表示でproviderが生成するラベルとエラーメッセージは英語固定とする。例は`5-hour`、`Weekly`、`Monthly tools`、`Weekly resets`、`Earliest expiry`。回数は数値だけを表示し、日本語の「回」は付けない。i18n用の通信拡張は追加しない。公式hostから返される接続先名・プラン名・モデル名はそのまま保持する。
